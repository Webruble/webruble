...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::WBR)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("WBR")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeWBRAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeWBRAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeWBRAmount = strAnonymizeWBRAmount.remove(strAnonymizeWBRAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeWBRAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeWBRAmount * COIN) nMaxToAnonymize = nAnonymizeWBRAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeWBRAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeWBRAmount));
        strAnonymizeWBRAmount = strAnonymizeWBRAmount.remove(strAnonymizeWBRAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeWBRAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
