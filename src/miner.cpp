...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("WBRMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("WBRMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("WBRMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("webruble-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("WBRMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running WBRMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadWBRMiner() exception");
    } catch (...) {
        LogPrintf("ThreadWBRMiner() exception");
    }

    LogPrintf("ThreadWBRMiner exiting\n");
}
